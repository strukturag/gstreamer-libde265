...
main (int argc, char *argv[])
{
  ...
  if (convert == NULL) {
    g_printerr ("Could not create convert element.\n");
    return -1;
  }

  GstElement *pngenc = gst_element_factory_make ("pngenc", "png");
  if (pngenc == NULL) {
    g_printerr ("Could not create pngenc element.\n");
    return -1;
  }
  ...
  g_object_set (G_OBJECT (sink), "location", "out%05d.png", NULL);
  ...
  */
  ...
  gst_bin_add_many (GST_BIN (pipeline), source, decoder, convert, pngenc, sink, NULL);

  gst_element_link_many (source, decoder, convert, pngenc, sink, NULL);
