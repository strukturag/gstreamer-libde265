...
process_get_cpu_stats(int64_t *user, int64_t *system)
{
    ...
    g_assert(fp != NULL);
    fscanf(fp, "%*d %*s %*c %*d %*d %*d %*d %*d %*u %*u %*u %*u %*u %lu %lu", &userinfo, &systeminfo);
    *user = userinfo;
    *system = systeminfo;
    fclose(fp);
}
...
main(int argc, char *argv[])
{
    ...
    printf("Global ticks:  user=%6ld system=%6ld\n", global_user_after - global_user_before, global_system_after - global_system_before);
    printf("Process ticks: user=%6ld system=%6ld\n", process_user_after - process_user_before, process_system_after - process_system_before);
    float user_cpu = 100 * (process_user_after - process_user_before) / ((float) global_user_after - global_user_before);
    float system_cpu = 100 * (process_system_after - process_system_before) / ((float) global_system_after - global_system_before);
    ...
    float fps = frames / ((float) time_total.tv_sec + (time_total.tv_usec / 1000000.0));
